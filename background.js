...
async function sendToContent(tabId, payload) {
  try {
    ...
    // Firefox MV3: content script may not be ready yet
...
async function ensureH1Tab() {
  ...
  if (active?.url?.startsWith("https://hackerone.com/")) {
    return active.id;
  }
...
api.runtime.onMessage.addListener(async (msg, sender) => {
  ...
  */
  ...
  if (msg?.type === "BEGIN_SCRAPE_FROM_INBOX") {
    const tabId = await ensureH1Tab();
    const templates = await loadQueryTemplates();

    await sendToContent(tabId, {
      type: "FETCH_REPORT_IDS_FROM_INBOX",
      inbox: msg.inbox,
